...
def main():
    ...
    parser.add_argument(
        "--sample_idx", type=int, default=0, help="Index of the sample to test"
    )
    ...
    if args.sample_idx >= len(test_data):
        print(f"Error: Sample index {args.sample_idx} out of range (0-{len(test_data)-1})")
        return

    item = test_data[args.sample_idx]
    idx = args.sample_idx
    print(f"Loaded sample {idx} from {len(test_data)} total samples")
    ...
    print(f"Verifier Answer: {result['verifier_answer']}")
