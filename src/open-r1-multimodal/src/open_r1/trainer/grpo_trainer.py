...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.reward_weights = kwargs.pop("reward_weights", None)
        assert len(self.reward_weights) == len(reward_funcs), "The number of reward weights must match the number of reward functions"
        ...
        ckpt_path = kwargs.get("pretrained_model_path", None) if kwargs.get("pretrained_model_path", None) else model_id
        
        # Check if loading from a LoRA checkpoint
        # First check if model_id itself is a LoRA adapter
        ...
        if os.path.isdir(model_id):
            ...
            if os.path.exists(adapter_config_path):
                ...
                print(f"[GRPO] Detected LoRA adapter at: {model_id}")
                print(f"[GRPO] Base model: {base_model_path}")
        
        # Also check ckpt_path if different from model_id
        if not is_lora_checkpoint and ckpt_path != model_id and os.path.exists(ckpt_path):
            ...
            if os.path.exists(adapter_config_path):
                ...
                with open(adapter_config_path, 'r') as f:
                    adapter_config = json.load(f)
                    base_model_path = adapter_config.get("base_model_name_or_path")
                print(f"[GRPO] Detected LoRA checkpoint at: {ckpt_path}")
                print(f"[GRPO] Base model: {base_model_path}")
        
        if is_lora_checkpoint:
            # Load base model first, then load LoRA weights for continued training
            print(f"[GRPO] Loading base model: {base_model_path}")
            model = model_cls.from_pretrained(base_model_path, **model_init_kwargs)
            print(f"[GRPO] Loading LoRA adapter from: {lora_adapter_path}")
            model = PeftModel.from_pretrained(model, lora_adapter_path, is_trainable=True)
            print("[GRPO] ✅ LoRA adapter loaded successfully for continued training")
        else:
            # Load full model checkpoint
            print(f"[GRPO] Loading full model from: {ckpt_path}")
            model = model_cls.from_pretrained(ckpt_path, **model_init_kwargs)

        # LoRA
        self.vision_modules_keywords = self.vlm_module.get_vision_modules_keywords()
        if peft_config is not None and not is_lora_checkpoint:
            # Only apply new LoRA if not already loaded from checkpoint
        ...
        if self.beta == 0.0:
            ...
            # Use Qwen2.5 VL registration only for Qwen models
            if "qwen" in model_id.lower():
                try:
                    ...
                    print(f"[WARN] Qwen2.5 VL registration failed ({e}); falling back to existing class registration.")
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        confidence_scores_collected = []  # Collect confidence scores from reward functions
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                # Prepare to collect confidence scores
                confidence_scores_out = []
                reward_kwargs['confidence_scores_out'] = confidence_scores_out
                
                # Add training_args to kwargs so reward functions can access it
                reward_kwargs['training_args'] = self.args
                
                output_reward_func = reward_func(prompts=prompts, completions=completions, **reward_kwargs)
                rewards_per_func[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device)
                
                # Store confidence scores if any were collected
                if confidence_scores_out:
                    confidence_scores_collected.extend(confidence_scores_out)
        ...
        # Log confidence scores if any were collected
        if confidence_scores_collected:
            # Filter out None values
            valid_confidence_scores = [s for s in confidence_scores_collected if s is not None]
            if valid_confidence_scores:
                confidence_tensor = torch.tensor(valid_confidence_scores, dtype=torch.float32, device=device)
                mean_confidence = self.accelerator.gather_for_metrics(confidence_tensor).mean().item()
                self._metrics["confidence_score"].append(mean_confidence)
                self._metrics["confidence_valid_ratio"].append(len(valid_confidence_scores) / len(confidence_scores_collected))
