...
class InvernVLModule(VLMBaseModule):
    ...
    def get_model_class(self, model_id: str, model_init_kwargs: dict):
        ...
        # Determine the actual model path for loading config
        ...
        if os.path.isdir(model_id):
            ...
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    ...
                    if "internvl" not in model_type and "internvl" not in arch_str:
                        raise ValueError(f"Not an InternVL model in config: {model_type}, architectures: {architectures}")
            else:
                ...
                if os.path.exists(adapter_config_path):
                    with open(adapter_config_path, 'r') as f:
                        ...
                        if base_model:
                            # Use base model for loading config
                            actual_model_id = base_model
                        else:
        ...
        self.model_config = AutoConfig.from_pretrained(actual_model_id, trust_remote_code=True)
    ...
    @staticmethod
    def get_question_template(task_type: str, use_confidence_score: bool = False):
        confidence_instruction = ""
        if use_confidence_score:
            confidence_instruction = " Additionally, provide a confidence score between 0 and 1 (as a decimal) enclosed within <confidence> </confidence> tags, indicating how confident you are in your answer."
        
        match task_type:
            case _:
                return "{Question} First output the thinking process in <think> </think> tags and then output the final answer in <answer> </answer> tags." + confidence_instruction
